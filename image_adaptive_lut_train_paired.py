...
parser.add_argument("--input_dir", type=str, default="/content/drive/MyDrive/", help="image directory")
...
os.makedirs("%s/saved_models" % opt.output_dir, exist_ok=True)
...
if opt.input_color_space == 'sRGB':
    dataloader = DataLoader(
        ImageDataset_sRGB(opt.input_dir, mode = "train"),
    ...
    psnr_dataloader = DataLoader(
        ImageDataset_sRGB(opt.input_dir,  mode="test"),
        batch_size=1,
        shuffle=False,
        num_workers=1,
    )
    ...
    dataloader = DataLoader(
        ImageDataset_XYZ(opt.input_dir, mode = "train"),
        batch_size=opt.batch_size,
        shuffle=True,
        num_workers=opt.n_cpu,
    )
    ...
    psnr_dataloader = DataLoader(
        ImageDataset_XYZ(opt.input_dir, mode="test"),
        batch_size=1,
        shuffle=False,
        num_workers=1,
    )
...
def visualize_result(epoch):
    ...
    classifier.eval()
    os.makedirs("%s/images/epoch%d/" % (opt.output_dir, epoch), exist_ok=True)
    for i, batch in enumerate(psnr_dataloader):
        real_A = Variable(batch["A_input"].type(Tensor))
        real_B = Variable(batch["A_exptC"].type(Tensor))
        ...
        mse = criterion_pixelwise(fake_B, real_B)
        psnr = 10 * math.log10(255.0 * 255.0 / mse.item())
        save_image(img_sample, "%s/images/epoch%d/%s.jpg" % (opt.output_dir, epoch, img_name[0]+'_'+str(psnr)[:5]), nrow=3, normalize=False)
...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    if epoch % opt.checkpoint_interval == 0:
        ...
        torch.save(LUTs, "%s/saved_models/LUTs_%d.pth" % (opt.output_dir, epoch))
        torch.save(classifier.state_dict(), "%s/saved_models/classifier_%d.pth" % (opt.output_dir, epoch))
        file = open('%s/saved_models/result.txt' % opt.output_dir,'a')
